...
public class MazeTest extends JFrame implements GridColors {
 ...
 //       Blob aBlob = new Blob(aGrid);
    ...
    public void solve() {
        ...
        // display all paths between source and destination. If no path then display "No path."
        ...
        // display minimum path between source and destination. If no path then display "No Minimum path."
        ArrayList<PairInt> minPath = m.findMazePathMin(0,0);
        if(minPath == null) {
            System.out.println("No Minimum path.");
        }


        // check if path exsist between source and destination. and if so display path in Path color.
